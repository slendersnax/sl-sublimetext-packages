    ...
    def run(self, edit, prefix):
        try:
            ...
            for sel in sels:
                ...
                label_template = '''label="%s%s%s%s"'''
                value_template = '''value="%s%s%s"'''
                ...
                print(max_label_index, vrange[max_label_index])
                ...
                if all_label_vals[max_label_index] < 10:
                    label_template = '''label="%s%s0%s%s"'''
                    value_template = '''value="%s0%s%s"'''
...
class makeCountryElementCommand(sublime_plugin.WindowCommand):
    def run(self):
        self.window.show_input_panel("Country prefix:", "", self.on_done, None, None)
